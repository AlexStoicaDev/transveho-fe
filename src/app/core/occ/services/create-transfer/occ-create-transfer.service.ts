...
import {CreateTransferData, CreateTransferStepperData, SelectedPassengers} from '../../models/create-transfer.model';
...
export class OccCreateTransferService {
  ...
  public getCreateTransferStepperData(
    ...
    let params = new HttpParams().append(
      'routeId', selectedPassengers.routeId.toString()
    ).append('selectedPassengersIds',
      selectedPassengers.selectedPassengersIds.join(', '));
    return this.http
      .get<CreateTransferStepperData>(this.getCreateTransferStepperDataEndpoint(), {
        params
      })
      .pipe(catchError(err => throwError(err)));
  }
