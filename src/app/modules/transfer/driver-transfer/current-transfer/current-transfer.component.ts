...
export class CurrentTransferComponent implements OnInit {
  ...
  constructor(
    ...
    this.passenger = this.currentTransfer.passengers[0];
    // var a = polyline.decode('_p~iF~ps|U_ulLnnqC_mqNvxq`@');
    // debugger
  }

  ngOnInit(): void {
    const pickUpAddress = this.currentTransfer.passengers[0].pickUpAddress;
    const destinationAddress = this.currentTransfer.passengers[0]
      .destinationAddress;
